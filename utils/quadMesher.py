...
def quadShapeFunction(xi, eta): # 左下，右下，右上，左上
    ...
    def __init__(self, mesh, material, bc):
        ...
        for line in f:
            ...
            self.edofMat[ctr,:] = np.array([[2*self.elemNodes[ctr,i],\
                                 ...
                                 for i in range(4) ]).reshape(-1);  # 编号为i的Node的DOF或fource编号：[2*i, 2*i+1](x方向/y方向)
            ctr += 1;
        ...
        for elem in range(self.numElems):
            ...
            self.elemArea[elem] = 0.5*np.abs(d1[0]*d2[1] - d1[1]*d2[0]) # 对于平行四边形，对角线向量叉积的模=2*面积
        self.elemArea = to_torch(self.elemArea)   # 似乎，对于梯形，对角线向量叉积的模=2*面积，也成立？
